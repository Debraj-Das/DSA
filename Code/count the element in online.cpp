...
template <class T, class V> class SegmentTree
{
  private:
    ...
    V getValue(int stIndex, int left, int right, int lo, int hi, V x)
    {
        if (left == lo && right == hi)
            return nodes[stIndex].getValue(x);

        int mid = (left + right) / 2;
        if (lo > mid)
            return getValue(2 * stIndex + 1, mid + 1, right, lo, hi, x);
        if (hi <= mid)
            return getValue(2 * stIndex, left, mid, lo, hi, x);

        V leftResult = getValue(2 * stIndex, left, mid, lo, mid, x);
        V rightResult = getValue(2 * stIndex + 1, mid + 1, right, mid + 1, hi, x);
  ...
  public:
    ...
    V getValue(int lo, int hi, int x)
    {
        return getValue(1, 0, N - 1, lo, hi, x);
    }
...
template <class T, class V> struct SegmentTree<T, V>::SegmentTreeNode
{
    ...
    void merge(SegmentTreeNode a, SegmentTreeNode b)
    {
        ...
        st.insert(st.end(), a.st.begin(), a.st.end());
        st.insert(st.end(), b.st.begin(), b.st.end());
    ...
    V getValue(int x)
    {
        ...
        int count = distance(st.begin(), upper_bound(all(st), x));
        return count;
    }
};

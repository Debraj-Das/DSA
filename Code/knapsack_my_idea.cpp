...
V<vi> dp;
int N, W;
int prices[5001], weight[5001];

int knapsack(int n, int w, int p)
{
    if (n == N)
    {
        return p;
    ...
    dp[n][w] = knapsack(n + 1, w, p);

    if (w + weight[n] <= W)
    {
        dp[n][w] = max(dp[n][w], knapsack(n + 1, w + weight[n], p + prices[n]));
    }
...
{
    cin >> N >> W;
    dp.resize(N, vi(W + 1, -1));

    f(i, 0, N) cin >> prices[i];
    f(i, 0, N) cin >> weight[i];

    cout << knapsack(0, 0, 0) << el;

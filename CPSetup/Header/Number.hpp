...
public:
  ...
  //~ Modulo Operations : gcd, add(+), sub(-), mul(*), div(/), lcm , inv(1/),
  //power(^)
  ...
  // Carmichael Number :
  // a composite number n which satisfies the modular arithmetic congruence
  // relation b^(n-1) = 1 (mod n) for all integers b which are relatively prime
  // to n
...
template <class T> T Number<T>::power(T a, T b) {
  T res = 1;
  while (b > 0) {
    if (b & 1)
      res = mul(res, a);
...
template <class T> T Number<T>::fact(T n) {
  T res = 1;
  for (T i = 2; i <= n; i++)
    res = mul(res, i);
  return res;
...
template <class T> T Number<T>::ncr(T n, T r) {
...
template <class T> T Number<T>::npr(T n, T r) {
  if (r > n)
    return 0;
  T res = 1;
  for (T i = 1; i <= r; i++)
...
template <class T> T Number<T>::nhr(T n, T r) {
  // nhr(n, r) = ncr(n + r - 1, r)
  if (n == 0 && r == 0)
    return 1;
  return ncr(n + r - 1, r);
...
template <class T> void Number<T>::extengcd(T a, T b, T res[]) {
  // ax + by = gcd(a, b)
...
template <class T> T Number<T>::mminv(T a, T b) {
...
template <class T> bool Number<T>::diophantine(T a, T b, T c, T res[]) {
  // ax + by = c
...
template <class T> bool Number<T>::isprime(T n) {
  if (n <= 1)
    return false;
  if (n <= 3)
    return true;
...
template <class T> std::vector<T> Number<T>::primeNumber(T n) {
...
template <class T> std::vector<T> Number<T>::factors(T n) {
  std::vector<T> res;
  for (T i = 1; i * i <= n; i++) {
    if (n % i == 0) {
      res.push_back(i);
...
template <class T> std::vector<T> Number<T>::primefactors(T n) {
  std::vector<T> res;
  while (n % 2 == 0) {
    res.push_back(2);
    n = n / 2;
...
template <class T> T Number<T>::phi(T n) {
...
template <class T> T Number<T>::catalan(T a) {
  T res = 1;
  for (T i = 1; i <= a; i++) {
    res = mul(res, a + i);
    res = div(res, i);
...
template <class T> T Number<T>::multiplicativeOrder(T a, T b) {
  T res = 1;
...
template <class T> T Number<T>::squareRoot(T a) {
  T l = 1, r = a, res;
  while (l <= r) {
    T mid = (l + r) / 2;
    if (mid * mid == a)
...
template <class T> T Number<T>::Legendre(T a, T b) {
  if (gcd(a, b) > 1)
    return 0;
...
template <class T> bool Number<T>::isCarmichaelNumber(T n) {
  if (isprime(n))
    return false;
  for (T i = 2; i < n; i++)
    if (power(i, n - 1) % n != 1)
...
template <class T> bool Number<T>::isPerfectSquare(T n) {
  T x = squareRoot(n);
  return x * x == n;
}
